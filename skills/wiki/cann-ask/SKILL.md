...
各 MCP 工具（`wiki_search` / `wiki_get_page` / `wiki_grep_raw` / `wiki_read_raw` / `wiki_help` / `wiki_submit_trajectory`）的详细契约见下方 §"MCP 工具契约详解"。
...
- **自动逐页 fetch + 合成带引用的答案** —— 对 top-3 static + top-2 dynamic 的 id 各调一次 `wiki_get_page(id)`（单页接口，循环取回），跨页面写实质内容（不是只抄 summary）
- **轨迹日志反馈 Q-Value**
...
**核心**：每次 cann-ask 调用 = 1 次 `wiki_search` + 每个目标页 1 次 `wiki_get_page`（单页接口，需循环；通常 top-3 static + top-2 dynamic ≈ 3-5 次）。query 是 30-150 字的聚焦自然语言提问（**不**抽实体、**不**挑 tags、**不**拆 sub-query）；`phase` 从 `device_feedback` 自动派生；`tags` 一律 `[]`；tier2 由 server 主 Agent 自动预跑(v5.2 已无 `include_dynamic` 客户端开关)。
...
**`wiki_get_page` 现在一次只取一页** —— 把 `static.results` top-3 和 `dynamic.results` top-2 的 id 收齐，**对每个 id 各调一次** `wiki_get_page(id)`（server 已不支持批量 `ids`）：
...
- **自然语言 query，单次检索** —— 一次 cann-ask = 1 次 `wiki_search` + 逐页 `wiki_get_page`（单页接口，每个目标页各调一次）；query 是 30-150 字的聚焦自然语言提问，**不**抽实体、**不**挑 tags、**不**拆 sub-query
...
- **path 字段已彻底下沉** —— `id` 是唯一锚点，绝不暴露 / 使用 `path`；fetch 时对 `static_top_3 + dynamic_top_2` 的每个 id **各调一次** `wiki_get_page(id)`（单页接口，循环）
- **raw 核对工具按需用** —— `wiki_grep_raw` / `wiki_read_raw` 是可选的真源佐证：把 API 签名 / 结构体字段 / 编译选项写进 kernel 前若怀疑 wiki 与官方源漂移,用它们核对;不是每次问答必调
